...
const PageButtonForm = () => {
  ...
  /// 변수 모음
  // 외부의 변수 불러오기
  const {data, handleDataChange, totalPages} = useContext(ItemListContext);
  ...
  const nowPage = data.page; // 현재 페이지
  const helperPage = data.page; // 페이지 번호를 만들 때 필요한 변수 -> 사용 안 함
  // 페이지 만드는 메서드
  function createPages() {
    // 출력 결과 담는 그릇 변수
    let result = undefined;// null로 해야하나?
    const resultNextDirectionNum = Math.ceil(nowPage / showedRangeOfPage); // 페이징박스를 하나의 페이지로 보는 numbe
    ...
    if(endPage === 1) {
      console.log(`endPage : ${endPage}`);
      result = <Pagination.Item active name="page" value={nowPage - 1}>{nowPage}</Pagination.Item>
      return result;
    }

    // 1 < endPage 아닐 때
    const firstPageInPagingBox = Math.floor((nowPage - 1) / showedRangeOfPage); // 페이징박스에서 첫번째 페이지 number
    ...
    if(nowPage > showedRangeOfPage) {
      // 이전 페이징 박스로 이동
      result += <Pagination.Prev name="page" value={nextPagingBox - 1} onClick={handleDataChange}/>
    }
    ...
    for(stopLine = 1; stopLine <= showedRangeOfPage; stopLine++) {
      ...
      if(countPage === nowPage) {
        result += <Pagination.Item name="page" active>{countPage}</Pagination.Item>
      } else {
        result += <Pagination.Item name="page" value={countPage - 1} onClick={handleDataChange}>{countPage}</Pagination.Item>
      }
    ...
    if(resultNextDirectionNum < endDirectionNum) {
      // 다음 페이징박스로 이동 >
      if(nowPage <= showedRangeOfPage) {
        result += <Pagination.Next value={(nextPagingBox + showedRangeOfPage * 1) - 1 } onClick={handleDataChange} />
      } else {
        result += <Pagination.Next value={(nextPagingBox + showedRangeOfPage * 2) - 1 } onClick={handleDataChange} />
      }
    }
