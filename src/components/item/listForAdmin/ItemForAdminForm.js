...
/**
 * Item of Item box component
 * writer : 이호진
 * init : 2023.03.28
 * updated by writer :
 * update :
 * description : 상품목록의 상품 관라지용 component
 *               props
 *                 > data : 상품 데이터 prop
 *                 > searchCond : 상품목록 찾기 위한 조건 데이터 prop
 *                 > numPosition : 현재 페이지에서 상품목록의 자리선정위한 번호 prop
 *                 > totalPages : 전체 페이지수 prop
 *                 > datasLength : 데이터 총 개수

 */
const ItemForAdminForm = ({data, numPosition, datasLength, totalPages, searchCond}) => {
  ...
  async function handleCancelClick(e) {
    ...
    if(result) {
      ...
      try {
        ...
        // 서버로 취소 요청
        const response = await cancel(itemNum);
        // 요청 성공
        // loding = false
        setLoding(false);
        console.log("요청 성공");
...
export default ItemForAdminForm;